...
REPO_URL = 'https://github.com/zkanda/superlist-tdd.git'
SITES_FOLDER = '/home/srvadmin/sites'


def deploy():
    _create_folder_structure_if_necessary(env.host)
...
def _create_folder_structure_if_necessary(site_name):
...
def _update_settings(source_folder, site_name):
    settings_path = path.join(source_folder, 'superlist/settings.py')
    sed(settings_path, "Debug = True", "Debug = False")
    append(settings_path, 'ALLOWED_HOSTS = ["%s"]' % (site_name,))
    secret_key_file = path.join(source_folder, 'superlist/secret_key.py')
...
def _update_static_files(source_folder):
    run('cd %s && ../virtualenv/bin/python3.3 manage.py collectstatic --noinput'
        % (source_folder,))


def _update_database(source_folder):
    run('cd %s ../virtualenv/bin/python3.3 manage.py syncdb --noinput' %
        (source_folder,))
